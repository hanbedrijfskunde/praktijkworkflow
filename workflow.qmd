...
def summarize_mermaid_diagram_to_html(diagram_code):
    ...
    for line in diagram_code.strip().split('\n'):
        ...
        if match:
            ...
            actions[actor].append(f"<li><strong>{current_phase}</strong>: {action}</li>")
            actions[target].append(f"<li><strong>{current_phase}</strong>: Receives action '{action}' from {actor}</li>")

    summary = {actor: sorted(activities) for actor, activities in actions.items()}

    # Generate HTML
    html_output = ["<html><body><ul>"]
    for actor, activities in summary.items():
        html_output.append(f"<li><strong>{actor}:</strong><ul>")
        html_output.extend(activities)
